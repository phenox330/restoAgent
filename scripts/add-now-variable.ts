...
async function addNowVariable() {
  if (!VAPI_API_KEY) {
    console.error("❌ VAPI_PRIVATE_KEY manquant");
    process.exit(1);
  }

  console.log(`🔧 Ajout de la variable {{ now }} au prompt...`);
...
Rappel: Sois humain(e), pas un robot. Les gens appellent un restaurant, pas un centre d'appels.`,
  ...
  console.log("✅ Variable {{ now }} ajoutée !");
  console.log("");
  console.log("📋 L'agent peut maintenant:");
  console.log("  - Calculer 'demain' à partir de la date actuelle");
  console.log("  - Calculer 'jeudi prochain' à partir de la date actuelle");
  console.log("  - Gérer toutes les dates relatives");
  console.log("");
  console.log("🧪 Teste avec: 'Je voudrais réserver pour demain soir'");
}

addNowVariable().catch(console.error);
